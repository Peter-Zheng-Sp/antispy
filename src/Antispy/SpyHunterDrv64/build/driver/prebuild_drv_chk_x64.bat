...
echo " ------ Enter Chk x64 Build Environment ------"
...
if "%target_os%" == "win7" (
	set DEFAULT_OUTPUT_FOLDER=objchk_win7_amd64
	set target_ostag=W7
	set DDK_DIR=%W7BASE%
) else if "%target_os%" == "wlh" (
	set DEFAULT_OUTPUT_FOLDER=objchk_wlh_amd64
...
SET DRIVER_ROOT=%BUILD_OUTPUT_DIR%amd64\%target_ostag%_Debug
cd /D %DRV_PROJECT_ROOT%
call %DDK_DIR%\bin\setenv %DDK_DIR% chk x64 %target_os% no_oacr
